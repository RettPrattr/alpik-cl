...
const TabsFilter = (props) => {
    ...
    // console.log(findImageUrl(Content[0].images, "url"), "URRRLRLRLLLLL")
    ...
    const getContent = async () => {
        ...
        .then((response) => {
            ...
            tabsDataPath === "abouts" ? setActualContent(content.data[0]) : setActualContent(content?.data)
    ...
    const handleClick = (el) => {
        ...
        let tempFilter = [];
        let tempEl;
        if (el.attributes.name === "Все проекты") {setActualContent(content)}
        // console.log(typeCategories, actualContent, "TVAR")
        if (el.attributes.name !== 'Все проекты') {
            content?.map((item) => {
                if (tabsCategoriesPath === "about-categories") {
                    item?.attributes.about_categories?.data?.map((obj) => {
    ...
    if (type === 1 && actualContent) {
        return (
            <div className='flex flex-col tabs tabs-1 container'>
                ...
                <div className='flex-row flex tabs-nav cd12'>
